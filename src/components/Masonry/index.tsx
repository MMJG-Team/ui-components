 ...
 * @returns 
 */
export const Masonry = React.forwardRef(function InternalMasonry<Item extends {
    id: string | number
    src: string
    naturalWidth: number
     ...
     * @param options 滚动选项
     */
    const scrollIntoView = (id: number | string, options: ScrollIntoViewOptions = {
        block: 'end',
        behavior: 'smooth',
    }) => {
        const target = containerRef.current?.querySelector(`.${ITEM_CLASS_NAME_PREFIX}-${id}`)
        target?.scrollIntoView(options)
    }

    const scrollBarCls = useScrollBar(() => containerRef.current!);
     ...
     */
    const { viewableItemsLayout, reLayout, masonryLayoutRef } = useMasonryLayout<Item>(containerRef, {
        gap,
        padding,
    ...
    useImperativeHandle(ref, () => ({
         ...
         * 滚动到顶部
         */
        scrollToTop: (behavior: ScrollBehavior = 'instant') => scrollToTop(behavior),
        /**
         * 滚动到指定元素
         * @param id 元素 id
         * @param options 滚动选项
         */
        scrollIntoView: (id: string | number, options: ScrollIntoViewOptions = {
            block: 'end',
            behavior: 'smooth',
    ...
    return (
        <div
            ...
            className={classNames(
                ...
                className,
                scrollBarCls,
                PlatformUtil.getBrowser().name === PlatformUtil.BrowserType.Safari && styles['scroll-thin']
            )}
            onScroll={onScroll}
        >
            {header}
                ...
                >
                    {viewableItemsLayout.map((columnItems, index) => (
                        <div
                            ...
                            style={{
                                height: masonryLayoutRef.current?.getColumnHeight(index) ?? 0
                            }}
                        >
            ...
            <div className={classNames(styles["masonry-footer"], footerClassName)}>
                {footer}
            </div>
        </div>
    )
